...
from .models import CheckInEntry, EntryTag, EntryTagRelation
from .serializers import CheckInEntrySerializer, CheckInEntryCreateSerializer
...
def entries_list_or_create(request):
    ...
    if request.method == 'GET':
        entries = CheckInEntry.objects.filter(user=request.user).order_by('-entry_date')
        ...
        # Create check-in entry with encryption
        ...
        try:
            ...
            if voice_file and entry.entry_type == 'voice':
                upload_result = cloudinary.uploader.upload(
                    voice_file,
                    resource_type='auto',
                    folder=f'assistant/voice/{request.user.id}',
            ...
            if video_file and entry.entry_type == 'video':
                upload_result = cloudinary.uploader.upload(
                    video_file,
                    resource_type='video',
                    folder=f'assistant/video/{request.user.id}',
        ...
        if NotificationService:
            try:
                ...
                # Update user stats
                request.user.total_entries = CheckInEntry.objects.filter(
                    user=request.user,
                    is_draft=False
                ).count()
...
def entry_detail_update_delete(request, entry_id):
    ...
    """
    try:
        entry = CheckInEntry.objects.get(id=entry_id, user=request.user)
