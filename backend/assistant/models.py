...
class CheckInEntry(models.Model):
    """
    Check-in entry model supporting text, voice, and video entries
    """
    ENTRY_TYPES = (
        ('text', 'Text'),
        ('voice', 'Voice'),
    ...
    user = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name='checkin_entries')
...
class EntryMedia(models.Model):
    """
    Media attachments for check-in entries (photos)
    """
    entry = models.ForeignKey(CheckInEntry, on_delete=models.CASCADE, related_name='media_files')
...
class EntryTag(models.Model):
    """
    Tags for categorizing check-in entries
    """
...
class EntryTagRelation(models.Model):
    ...
    """
    entry = models.ForeignKey(CheckInEntry, on_delete=models.CASCADE, related_name='entry_tags')
